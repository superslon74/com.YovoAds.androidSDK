...
public class BannerExchange extends ExampleBanner
{
    private BannerExchange mc_this = null;
    private IAdUnitOnMethod   m_callback = null;
    private YBanner m_banner = null;


    public BannerExchange(IAdUnitOnMethod _callback, String _empty, EGravity _eGravity, int _showTime)
    {
        m_callback = _callback;
        SetShowTimeMax(_showTime);
        me_isAdUnitlLoadingResult = EAdUnitLoadingResult._NONE;
        YovoSDK.ShowLog("BannerExchange", "init");
        Create("", _eGravity);
        mc_this = this;
    }
    ...
    public void Create(final String _adUnitId, final EGravity _eGravity)
    {
        DI.m_activity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                m_banner = new YBanner(EAdNetworkType._EXCHANGE, new IAdUnitOnMethod() {
        ...
        if (_isPause) {
            if (GetShowTimeCur() > 0) {
                if (YTimer.getInstance().BannerSetPause(_isPause)) {
                    YovoSDK.ShowLog("BannerExchange", "SetPause= " + GetShowTimeCur());
                    return true;
                }
            }
            return false;
