...
public class RewardExchange extends ExampleReward
{
    private IAdUnitOnMethodReward m_callback = null;
    private YReward m_reward;

    public RewardExchange (IAdUnitOnMethodReward _callback, String _empty)
    {
        m_callback = _callback;
        me_isAdUnitlLoadingResult = EAdUnitLoadingResult._NONE;
        YovoSDK.ShowLog("RewardExchange", "init");
        Create("");
    }
    ...
    public void Create(final String _adUnitId) {
        ...
        DI.m_activity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                m_reward = new YReward(EAdNetworkType._EXCHANGE, new IAdUnitOnMethodReward() {
