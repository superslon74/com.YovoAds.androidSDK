...
public class RewardCrossPromotion extends ExampleReward
{
    private IAdUnitOnMethodReward m_callback = null;
    private YReward m_reward;

    public RewardCrossPromotion (IAdUnitOnMethodReward _callback, String _empty)
    {
        m_callback = _callback;
        me_isAdUnitlLoadingResult = EAdUnitLoadingResult._NONE;
        YovoSDK.ShowLog("RewardCrossPromotion", "init");
        Create("");
    }
    ...
    public void Create(final String _adUnitId) {
        ...
        DI.m_activity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                m_reward = new YReward(EAdNetworkType._CROSS_PROMOTION, new IAdUnitOnMethodReward() {
