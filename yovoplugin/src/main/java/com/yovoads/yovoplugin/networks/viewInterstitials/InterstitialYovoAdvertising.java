...
public class InterstitialYovoAdvertising extends ExampleInterstitial {
    private IAdUnitOnMethod m_callback = null;
    private YInterstitial m_interstitial;

    public InterstitialYovoAdvertising(IAdUnitOnMethod _callback, String _empty) {
        m_callback = _callback;
        me_isAdUnitlLoadingResult = EAdUnitLoadingResult._NONE;
        YovoSDK.ShowLog("InterstitialYovoAdvertising", "init");
        Create(_empty);
    }
    ...
    public void Create(final String _adUnitId) {
        ...
        DI.m_activity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                m_interstitial = new YInterstitial(EAdNetworkType._YOVO_ADVERTISING, new IAdUnitOnMethod() {
